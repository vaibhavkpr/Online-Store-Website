...
class Ships_to{
    ...
    function readOne(){
        ...
        if($row){
            ...
            $this->receive_date=$row['receive_date'];
        }
    }
    ...
    function create(){
        ...
        if($row){
            ...
            $query = "INSERT INTO " .$this->table_name. " SET customer_id=:customer_id, order_id=:order_id, branch_id=:branch_id, receive_date=:receive_date";
            ...
            $statement->bindParam(":receive_date", $this->receive_date);
    ...
    function update(){        
        ...
        // Check if ships_to exists before creating it
        $check_query = "SELECT customer_id FROM " .$this->table_name. " WHERE customer_id=" .$this->customer_id. "";
        $statement = $this->db->prepare($check_query);
        $statement->bindParam(1, $this->customer_id);
        $statement->execute();
    ...
    function delete(){
        ...
        // Check if ships_to exists before deleting it
        $check_query = "SELECT customer_id FROM " .$this->table_name. " WHERE customer_id=" .$this->customer_id. "";
        $statement = $this->db->prepare($check_query);
        $statement->bindParam(1, $this->customer_id);
        $statement->execute();
        ...
        if(!$row){
            ...
            // Bind customer_id of the ships_to to be deleted
            $statement->bindParam(1, $this->customer_id);
